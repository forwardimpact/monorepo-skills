...
One input sets the active tracker: the environment variable
...
- `open-change` — propose a diff that carries an approval gate.
...
- `close` — close an issue or change without a merge.
...
**Dispatch bridge (github).** On github, the `kata-dispatch` reactor handles
...
root has a generic name, so an agent that reads the tree infers the active
store without this file:
...
merge time. It is not a materialized patch.
...
- **Remote-git steps are no-ops (filesystem).** `open-change` and
  ...
  merge time. Neither needs a network, remote, or git, only plain file writes.
- **Out-of-band trust (filesystem).** The `approval` field records which signal
  granted it. Trust that the setter is authorized is out-of-band. The actor
  that runs `gate` is trusted by construction (the benchmark harness or a
  human). The tracker never emulates contributor-list trust offline.
